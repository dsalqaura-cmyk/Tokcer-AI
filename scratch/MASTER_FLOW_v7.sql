-- ============================================================
-- 🏮 TOKCER AI: MASTER FLOW ACTIVATION (v7)
...
BEGIN
    ...
    IF v_user_id IS NULL THEN
        ...
        VALUES (gen_random_uuid(), v_user_id, json_build_object('sub', v_user_id, 'email', p_email), 'email', NOW(), NOW(), NOW());
    ELSE
    ...
    UPDATE public.profiles SET plan = 'ultimate' WHERE id = v_user_id;

    -- D. Send Email
    SELECT value INTO v_resend_api_key FROM public.ai_configs WHERE key = 'resend_api_key';
    v_html := '<div style="font-family:sans-serif; background:#000; color:#fff; padding:40px; border-radius:20px;">' ||
              '<h2>Selamat Datang Partner!</h2><p>Akun Partner & User Ultimate (60 Hari) Anda telah aktif.</p>' ||
              '<div style="background:#111; padding:20px; border:1px dashed #444;">' ||
              'Email: <b>' || p_email || '</b><br>Password: <b>' || p_password || '</b></div><br>' ||
              '<a href="https://staging.tokcer-ai.com/login" style="background:#ea580c; color:#fff; padding:12px 24px; text-decoration:none; border-radius:8px;">LOGIN SEKARANG</a></div>';
    
    IF v_resend_api_key IS NOT NULL THEN
...
BEGIN
    ...
    IF v_user_id IS NOT NULL THEN UPDATE public.profiles SET plan = 'ultimate' WHERE id = v_user_id; END IF;
